...
int bluepy_connect(const char* mac_address, uint8_t mac_type)
{
...
#endif
    g_free(opt_dst);
    opt_dst = g_strdup(mac_address);

    g_free(opt_dst_type);
    if (mac_type != BDADDR_LE_PUBLIC) {
...
static gboolean hci_monitor_cb(GIOChannel *chan, GIOCondition cond, gpointer user_data)
{
    ...
    case HCI_COMMAND_PKT:
    {
        ...
        {
            if (err)
                DBG("g_io_channel_read_chars() reports state %d: %s", r, err->message);
            return TRUE;
        }
